...
- **Unified Fullstack Monorepo vs. Separate Frontend & Backend Repositories (Multi-repo)**: We chose to consolidate both applications (frontends) and the shared database/service layers (backends) into a single codebase rather than split them into two independent repositories. This guarantees **end-to-end type safety** (database schemas/types flow directly to UI components), eliminates **Server-to-Server network latency** (React Server Components query the database directly), simplifies DevOps (no CORS configuration, single CI/CD pipeline), and enables sharing input validation rules (Zod schemas). We trade off technology stack flexibility and decoupled release lifecycles to maximize development speed, unified deployment, and compile-time correctness.
