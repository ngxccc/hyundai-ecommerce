...
- `area:auth` — Authentication, sessions, user accounts, roles, and permissions.
- `area:catalog` — Products, categories, brands, specifications, faceted search.
- `area:order` — Checkout flow, order state transitions, line items, outbox events.
- `area:quote` — B2B quote negotiation, pricing tiers, dealer discounts, Excel export.
- `area:payment` — PayOS integration, payment transactions, webhooks, debt tracking.
- `area:warehouse` — Inventory stock management, warehouse allocations.
- `area:portal` — Dealer/Contractor/B2B portal, debt tracking, employee management.
- `area:admin` — Backoffice admin portal, catalog forms, Cloudinary media upload.
- `area:storefront` — Next.js 16 customer storefront UI, RSCs, `"use cache"`, Server Actions.
- `area:db` — PostgreSQL schemas, Drizzle ORM migrations, indexes, DTOs, pure services.
- `area:infra` — Turborepo, Doppler, Redis, Docker, CI/CD workflows, cron handlers.
- `area:harness` — Agent harness, context, skills, prompts, standards.
- `area:api` — REST API routing, pipes, filters, OpenAPI, versioning.
