...
import { getUpdateWarehouseStockSchema } from "@nhatnang/database/validators";
...
export async function setProductStockAction(data: {
  ...
  try {
    ...
    // Validate
    const schema = getUpdateWarehouseStockSchema((key) => key);
    ...
    if (error instanceof AuthError) {
      const message = error.message === "UNAUTHORIZED" ? t("unauthorized") : t("forbidden");
      return { success: false as const, error: message };
    }
    console.error("[setProductStockAction]", error);
    
    let errorMessage = t("updateWarehouseStockFailed");
    if (error instanceof Error && error.message === "errors.updateWarehouseStockFailed") {
