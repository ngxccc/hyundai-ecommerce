...
To ensure our application maintains an authentic, high-precision industrial enterprise aesthetic (mirroring Linear, Stripe, and Hyundai Global Engineering Portals) rather than looking like an unpolished AI template:
...
- **RULE**: Gradients (`bg-gradient-to-*`, `linear-gradient`, `radial-gradient`) are **STRICTLY FORBIDDEN** on:
  - Background panels, hero sections, and auth sidebars.
  - Button fills, card surfaces, table headers, and badges.
...
- **Success (`emerald`)**, **Error (`destructive`)**, and **Warning (`amber`)** are reserved strictly for:
  ...
  - Badge text labels on order/quote statuses (`STATUS: COMPLETED`, `STATUS: CANCELLED`).
