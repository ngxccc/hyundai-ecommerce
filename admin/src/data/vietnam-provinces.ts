...
/**
 * Danh mục Đơn vị Hành chính Việt Nam - Phiên bản 2 (Sau sắp xếp/sáp nhập 07/2025)
 * Cấu trúc 2 cấp tinh gọn:
 * - Cấp 1: 34 Tỉnh / Thành phố trực thuộc Trung ương
 * - Cấp 2: Phường / Xã / Thị trấn trực thuộc
 */
export const VIETNAM_PROVINCES_V2: ProvinceV2[] = [
  {
...
export function getProvinceNames(): string[] {
  return VIETNAM_PROVINCES_V2.map((p) => p.name);
}

export function getSubUnitsByProvinceName(provinceName: string): string[] {
  const found = VIETNAM_PROVINCES_V2.find((p) => p.name === provinceName);
  return found ? found.subUnits : [];
}
