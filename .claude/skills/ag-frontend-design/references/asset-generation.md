...
Complete workflow for generating design-aligned visual assets using Claude's built-in multimodal vision.
...
Use Claude's built-in multimodal vision for image generation:
...
Use Claude's built-in multimodal vision to analyze the generated asset (see `visual-analysis.md` for complete workflow):
...
4. Consider using imagemagick CLI or browser Canvas API for post-generation adjustments
