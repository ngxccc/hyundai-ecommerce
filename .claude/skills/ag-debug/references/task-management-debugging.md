...
This reference is optional coordination guidance for complex investigations. It does not replace the `debugger` agent as the owner of root-cause analysis, and it does not replace `execute-agent` as the owner of implementation.
...
Debug tasks are **separate from** planning/execution phase tasks.

**When an execution workflow spawns debugger:**

1. Execute workflow encounters failing tests → creates debug pipeline tasks
2. Debug pipeline executes (assess → collect → analyze → fix → verify)
3. All debug tasks complete → the execution workflow marks debugging complete
4. Execution proceeds to the next phase
