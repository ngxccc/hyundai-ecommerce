...
Track optional parallel scout worker execution via task metadata when your runtime exposes task-tracking tools.
...
TaskCreate(
  ...
  metadata: {
    agentType: "research-agent", // "research-agent" (internal) or "external-cli"
...
- `agentType` — Worker type: `"research-agent"` for internal or `"external-cli"` for local external tooling
...
TaskCreate(subject: "Scout src/auth/ for auth files",
  ...
  metadata: { agentType: "research-agent", scope: "src/auth/", scale: 6,
...
// ... launch all internal scout workers
...
TaskCreate(subject: "Scout db/ for migrations via gemini",
  ...
  metadata: { agentType: "external-cli", scope: "db/,migrations/", scale: 3,
...
Scout tasks are **independent** from planning/execution phase tasks — NOT parent-child.
...
4. The planning workflow hydrates phase tasks separately from scout tasks
