# Internal Scouting with Shell Search and Optional Research Agents

Use local shell search first. Add parallel `research-agent` workers only when the search space is broad enough to justify it.

## How It Works

Start with `rg`, `rg --files`, `glob`, and targeted file reads. For larger audits, split directories across multiple `research-agent` workers.
...
- Spawn all agents in one parallel round when the search space is clearly split
