...
try {
  ...
  const writeTools = ['write_to_file', 'replace_file_content', 'multi_replace_file_content', 'Write', 'Edit', 'MultiEdit'];
  
  if (writeTools.includes(toolName)) {
    // Determine the target file path depending on the tool interface
    const targetFile = toolInput.TargetFile || toolInput.file_path || toolInput.path || '';

    if (targetFile) {
      const isInternalPath = targetFile.includes('.gemini/antigravity-cli/brain/') || 
                             targetFile.includes('.claude/brain/') || 
                             targetFile.includes('appDataDir/brain/');

      const isForbiddenFile = targetFile.endsWith('task.md') || 
                              targetFile.endsWith('walkthrough.md') || 
                              targetFile.endsWith('implementation_plan.md');

      if (isInternalPath && isForbiddenFile) {
        // Block the action
        const errorMessage = `[STRICT PATH ENFORCEMENT] You are strictly forbidden from writing to ${targetFile}. ` +
